...
function Sidebar() {
	const { navigation, setNavigation, setIsEditMode, setCurrentFloor } = useContext(NavigationContext) as NavigationContextType;
	...
	return (
		<aside className="flex flex-col rounded-none w-[35rem] h-screen p-3 bg-white shadow-xl shadow-gray-200 -translate-x-full transform transition-transform duration-150 ease-in lg:translate-x-0 lg:shadow-md ">
			<header className="flex flex-col mb-4 pr-1 border-b py-2 w-full">
				<a href="https://github.com/openindoormap/openindoormaps" target="_blank" rel="noreferrer" className="font-semibold text-blue-800 text-xs">
			...
			<div className="flex mb-3 bg-gray-100 rounded-lg p-0.5">
				<button
					...
					className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
						activeTab === "places" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"
					}`}
				>
					🏬 Places
				</button>
				<button
					...
					className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
						activeTab === "desks" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"
					}`}
				>
					🪑 Desks
				</button>
								...
								<header className="p-2">
									<h2 className="text-2xl font-bold">
										...
										<span className="ml-2 text-sm font-medium text-gray-900">
											- {parsedObjects[letter].len} {parsedObjects[letter].len === 1 ? "Result" : "Results"}
										</span>
									</h2>
								</header>
								<div className="flex flex-col ">
										...
										>
											<div className="m-1">
												<p className="text-xs 2xl:text-sm font-semibold">
													{item.name}
													{item.floor && <span className="text-gray-400 ml-1 font-normal">(F{item.floor})</span>}
												</p>
