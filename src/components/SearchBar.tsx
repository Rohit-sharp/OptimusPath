...
import { navigateToObject, navigateWithDelay as navigationTestAll } from "@/utils/navigationHelper";
...
function SearchBar() {
	...
	const { navigation, setNavigation, setCurrentFloor } = useContext(NavigationContext) as NavigationContextType;
	const { setIsEditMode } = useContext(NavigationContext) as NavigationContextType;
	...
	function handleInputChange(event: React.ChangeEvent<HTMLInputElement>) {
		...
		if (value) {
			...
			const filteredSuggestions = objects.filter((obj) => obj.name.toLowerCase().includes(value.trim().toLowerCase()));
	...
	function handleKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
		if (isAutocomplete) {
			if (event.key === "ArrowDown") {
				...
				void navigateToObject(suggestions[selectedIndex].name, navigation, setNavigation, setCurrentFloor);
			}
		}
	}
	...
	function handleSearch(inputValue: string) {
		const matchingObject = objects.find((obj) => obj.name.toLowerCase() === inputValue.trim().toLowerCase());
		if (!matchingObject) {
			if (inputValue === "Test") {
				const delay = 500;
		...
		void navigateToObject(matchingObject.name, navigation, setNavigation, setCurrentFloor);
		setSelectedIndex(-1);
	}
		...
		<div className="md:w-96 w-full flex flex-row">
			<div className="flex flex-inline rounded w-full">
				...
				<div className="flex w-full relative">
					<input
						className={`h-12 p-4 w-full flex-none text-gray-900 text-sm md:text-md ${isInputInvalid && "input-error"} `}
						placeholder="Search"
						value={inputValue}
								...
								suggestions.map((obj, index) => (
									<li
										...
										className={`cursor-pointer p-2 border-t text-gray-600 hover:bg-gray-200 active:bg-gray-300 text-sm md:text-md ${
											selectedIndex === index ? "border-l-4 border-blue-500 bg-gray-200" : ""
										}`}
										onMouseDown={() => handleSuggestionClick(obj)}
									>
										<span>{obj.name}</span>
										{obj.floor && <span className="text-xs text-gray-400 ml-2">F{obj.floor}</span>}
									</li>
								))
							) : (
								<li className="p-2 text-gray-500 text-sm md:text-md" key={"no-result"}>
									No results found
								</li>
							)}
						</ul>
