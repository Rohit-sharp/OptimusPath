...
import { navigateToObject, drawPathForFloor, lastCalculatedRoute } from "@/utils/navigationHelper";
...
function IndoorMapWrapper() {
	...
	// When the floor changes, redraw the cached route on the new floor.
	useEffect(() => {
		if (lastCalculatedRoute && navigation.end) {
			setTimeout(() => {
				drawPathForFloor(lastCalculatedRoute!.nodes, currentFloor);
			}, 100);
		}
	}, [currentFloor]);
	...
	function handleNavigationClick() {
		setModalOpen(false);
		void navigateToObject(object.name, navigation, setNavigation, setCurrentFloor);
	}
	...
	return (
		<div className="relative w-full h-full bg-white center overflow-hidden">
			<ObjectDetailsModal open={modalOpen} object={object} onClose={() => setModalOpen((cur) => !cur)} objectNavigation={handleNavigationClick} />

			<BookingDialog open={bookingDialogOpen} desk={selectedDesk} onClose={() => setBookingDialogOpen(false)} />
				...
				<TransformComponent wrapperClass="bg-white">
					<MapBackground currentFloor={currentFloor}>
						<Objects
							...
							className={isEditMode ? "" : "hover:cursor-pointer hover:opacity-50"}
							currentFloor={currentFloor}
						/>
						{/* Desk markers on the map */}
						{bookingCtx && <DeskMarkers desks={bookingCtx.desks} currentFloor={currentFloor} onDeskClick={handleDeskClick} />}
						<Paths currentFloor={currentFloor} />
						<Positions
							positionRadius={positionRadius}
							handlePositionClick={handlePositionClick}
							className={isEditMode ? "opacity-100 cursor-pointer hover:fill-[#488af4] " : "opacity-0"}
