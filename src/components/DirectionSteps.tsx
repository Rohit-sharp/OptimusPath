import { lastCalculatedRoute } from "@/utils/navigationHelper";
...
function DirectionSteps({ onFloorChange }: DirectionStepsProps) {
	if (!lastCalculatedRoute) return null;

	const steps = lastCalculatedRoute.directions;
			...
			<ul className="space-y-1">
				{steps.map((step, idx) => (
					<li key={idx} className="flex items-start gap-2 text-sm">
						...
						<span
							className={`${step.text.includes("Elevator") ? "text-amber-600 font-medium cursor-pointer hover:underline" : "text-gray-700"}`}
							onClick={() => {
